	...
	public void pullTrxs() throws ProcessingException, ServiceException, RepositoryException {
		...
		try {
			...
			if (!StringUtils.equalsIgnoreCase(env.getActiveProfiles()[0], "prod")) {
				String active=NibblerDirectoryStatus.ACTIVE.name()+NibblerDirectoryStatus.ACTIVE_NO_LOAN_ACCT.name()+NibblerDirectoryStatus.ACTIVE_NO_ROUNDUP.name();
				accts=accts.stream().filter(a-> active.contains(a.getNibbler().getNibblerDir().getStatus())).collect(Collectors.toList());
				accts.forEach(nibblerAccount -> {
					for (int i = 0; i < 5; i++) {
						try {
							...
							System.out.println("integrationSao.addTestTx success");
						} catch (Exception e) {
			...
			for (NibblerAccount acct : accts) {
				if (acct.getUseForRounding()) {
					try {
						List<Transaction> extTrxs = integrationSao.retrieveTransactions(
		...
		try {
			...
			customers.forEach(c -> {
				try {
					...
					System.out.println("Customer deleted ===>"+c.getId());
				} catch (Exception e) {
					System.out.println("error Customer deleted ===>"+c.getId());
					e.printStackTrace();
				}
			});
		} catch (ServiceException e1) {
			System.out.println("error loading all customer from finicity");
			e1.printStackTrace();
		}
