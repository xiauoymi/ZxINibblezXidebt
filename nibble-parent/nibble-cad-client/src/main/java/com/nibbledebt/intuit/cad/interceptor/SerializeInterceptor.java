...
public class SerializeInterceptor implements Interceptor {
	...
	public void execute(IntuitMessage intuitMessage) throws AggCatException {
		...
		String methodType = (String) requestElements.getRequestParameters()
				.get("method-type");

		if ((methodType.equals(MethodType.POST.toString()))
				|| (methodType.equals(MethodType.PUT.toString()))) {
			ISerializer serializer = SerializerFactory.getSerializer(intuitMessage.getRequestType());
