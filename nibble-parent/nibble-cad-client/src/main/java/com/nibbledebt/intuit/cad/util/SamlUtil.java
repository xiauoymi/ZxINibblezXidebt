...
public class SamlUtil {
	...
	public Map<String, String> getSamlResponse(String signedSamlStr)
		...
		LOG.debug(new StringBuilder().append("Signed saml str=")
				.append(signedSamlStr).toString());
		...
		try {
			...
			connection.setRequestProperty("Content-Length",
					Integer.toString(signedSamlStr.getBytes().length));
			connection.setRequestProperty("Content-Language", "en-US");
			connection.setRequestProperty("Authorization",
					new StringBuilder().append("OAuth oauth_consumer_key=\"")
							.append(this.consumerKey).append("\"").toString());
			...
			writer.writeBytes(new StringBuilder().append("saml_assertion=")
					.append(signedSamlStr).toString());
			writer.flush();
			writer.close();

			int respCode = connection.getResponseCode();
			LOG.debug(new StringBuilder().append("Response code=")
					.append(respCode).toString());
			if (respCode == 200) {
		...
		LOG.debug(new StringBuilder().append("OAuth access tokens = ")
	...
	public String createSignedSAMLPayload() throws SamlAssertionException {
		LOG.debug(new StringBuilder().append("Signing for subject = ")
				.append(this.subject).append("; Saml provider id=")
				.append(this.samlProviderId).toString());
		...
		try {
			SAMLCredentials cred = new SAMLCredentials(
					...
					Config.getProperty("saml.keyPassword"));
