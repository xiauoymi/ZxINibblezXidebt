...
public class SAMLCredentials {
	...
	public SAMLCredentials(String keystoreFile, String keyStorePassword,
			String alias, String keyPassword) throws AggCatException {
		InputStream is = null;
		try {
			if ((keystoreFile.indexOf('/') != -1)
					|| (keystoreFile.indexOf('\\') != -1))
			...
			if (is == null) {
				throw new AggCatException("Could not get resource: "
						+ keystoreFile);
			}
			this.credential = loadCredential(is, keyStorePassword, alias,
					keyPassword);
		} catch (Exception e) {
			throw new AggCatException(
					"Exception while reading the certificate file : "
							+ keystoreFile, e);
		} finally {
			if (is != null)
				try {
	...
	private BasicX509Credential loadCredential(InputStream inputstream,
			String keystorePassword, String certAlias, String keyPassword)
		...
		try {
			...
			KeyStore.PrivateKeyEntry privatekeyentry = (KeyStore.PrivateKeyEntry) keystore
					.getEntry(certAlias, passwordprotection);
			LOG.debug("loading cred 2.");
			if (privatekeyentry != null) {
				PrivateKey privateKey = privatekeyentry.getPrivateKey();
				LOG.debug("loading cred 3.");
				X509Certificate x509certificate = (X509Certificate) privatekeyentry
						.getCertificate();
