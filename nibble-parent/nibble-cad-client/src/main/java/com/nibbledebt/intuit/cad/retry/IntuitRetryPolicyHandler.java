...
public class IntuitRetryPolicyHandler implements HttpRequestRetryHandler {
	...
	public IntuitRetryPolicyHandler(int retryCount, int retryInterval)
		...
		RetryHelper.argumentNotNegativeValue(retryInterval,
	...
	public IntuitRetryPolicyHandler(int retryCount, int initialInterval,
			int increment) throws AggCatException {
		RetryHelper.argumentNotNegativeValue(retryCount,
				"retry.incremental.count");
		RetryHelper.argumentNotNegativeValue(initialInterval,
				"retry.incremental.interval");
		RetryHelper.argumentNotNegativeValue(increment,
				"retry.incremental.increment");
	...
	public IntuitRetryPolicyHandler(int retryCount, int minBackoff,
			int maxBackoff, int deltaBackoff) throws AggCatException {
		RetryHelper.argumentNotNegativeValue(retryCount,
				"retry.exponential.count");
		RetryHelper.argumentNotNegativeValue(minBackoff,
				"retry.exponential.minBackoff");
		RetryHelper.argumentNotNegativeValue(maxBackoff,
				"retry.exponential.maxBackoff");
		RetryHelper.argumentNotNegativeValue(deltaBackoff,
				"retry.exponential.deltaBackoff");
	...
	private boolean checkPolicy(int executionCount) {
		...
		if (this.mechanism.equalsIgnoreCase("incrementalretry")) {
			if (executionCount < this.retryCount) {
				try {
					Thread.sleep(this.initialInterval + this.increment
							* executionCount);
					return true;
				} catch (Exception e) {
					LOG.error(e.getMessage());
		...
		if (this.mechanism.equalsIgnoreCase("exponentialretry")) {
			if (executionCount < this.retryCount) {
				try {
					int delta = (int) ((Math.pow(2.0D, executionCount) - 1.0D)
							* (this.deltaBackoff * 0.8D) + (Math.random()
							* (this.deltaBackoff * 1.2D) - this.deltaBackoff
							* 0.8D + 1.0D));

					int interval = Math.min(this.minBackoff + delta,
							this.maxBackoff);
