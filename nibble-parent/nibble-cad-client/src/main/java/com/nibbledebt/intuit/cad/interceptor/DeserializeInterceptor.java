...
public class DeserializeInterceptor implements Interceptor {
	...
	public void execute(IntuitMessage intuitMessage) throws AggCatException {
		...
		HttpResponse httpResponse = intuitMessage.getResponseElements()
				.getHttpResponse();

		Object response = deserialize(httpResponse, intuitMessage
				.getRequestElements().getResourceTypeIdentifier(),
				intuitMessage.getResponseElements().getResponseStream());
	...
	public Object deserialize(HttpResponse httpResponse,
			String resourceTypeIdentifier, InputStream content)
		...
		if (contentTypeHeader != null) {
			...
			for (String contentTypePart : contentTypes) {
				if (contentTypePart.matches("(.*)/(.*)")) {
					if (contentTypePart.equalsIgnoreCase(ContentType.XML
							.toString()) || contentTypePart.equalsIgnoreCase(ContentType.JSON
									.toString())) {
						String serializeFormat = contentTypePart.substring(
								contentTypePart.lastIndexOf("/") + 1,
								contentTypePart.length());
						LOG.info("HttpResponse content-type (deserialization format) : "
								+ serializeFormat);

						if (StringUtils.hasText(serializeFormat)) {
							ISerializer serializer = SerializerFactory
									.getSerializer(serializeFormat);
							Class resourceType = ResourceTypeLocator
									.getResourceType(resourceTypeIdentifier);
							response = serializer.deserialize(
									StringUtils.getContent(content),
									resourceType);
						}
					} else if (contentTypePart
							.equalsIgnoreCase(ContentType.OCTETSTREAM
									.toString())) {
						response = content;
					} else if ((contentTypePart
							.equalsIgnoreCase(ContentType.HTML.toString()))
							|| (contentTypePart
									.equalsIgnoreCase(ContentType.TEXT
											.toString()))) {
