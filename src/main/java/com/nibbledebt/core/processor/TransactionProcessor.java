...
import com.nibbledebt.integration.model.plaid.Transaction;
			...
			if(nacct.getUseForRounding()){
				for(int i=7; i>0; i--){
					...
					for(AccountTransaction trx : trxsForDay){
						...
						dtrx.setInstitutionName(trx.getAccount().getInstitutionName());
		...
		try {
			...
			for(NibblerAccount acct : accts){
				if(acct.getUseForRounding()){
					...
					TransactionsResponse tresp = extAccountSao.retrieveTransactions(acct.getNibbler().getExtAccessToken(), acct.getExternalId(), acct.getLastTransactionPull()==null ? null : timeformat.format(acct.getLastTransactionPull()));
